...
int main()
{
    ...
    Write_to_file("initial\\kinsol1", "val", Cell_Properties_vector, Cell_Properties_inter);
    for (int i = 0; i < Nx; i++)
    {
        if (i <= preinter) {
    ...
    koeff_params.open("params\\koeffs.dat");
    koeff_params << R"(VARIABLES= "T, K", "Ld", "Pf", "Lambda", "Cp", "rho")";
    koeff_params << "TITLE=\"" << "Graphics" << "\"" << endl;
    t_curr = 1;
    for (int i = 1; i < N_x; i++)
    {
        double T_tmp = 200 + (700 - 200) / (N_x - 1) * i;
