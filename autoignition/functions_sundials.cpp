...
void InitialData(int& Nx, vector<double>& x_vect, vector<Cell_Properties>& Cell_Properties_vector, double Tstart, double Tfinish, double* Ystart, double* Yend)
{
    ...
    //Xi[komponents["O2"]] = 0.0001;
    //Xi[komponents["N2"]] = 1 - Xi[komponents["O2"]];
    ...
    Ystart[komponents["O2"]] = (1. - Ystart[komponents[Fuel]]);
    ...
    double rho0_g = get_rho(Ystart, Tright, 'g');
    double R = ri * pow(rho0_d / rho0_g / 0.062 / phi, 1. / 3.);
    int x_eps = 50;
    double h_dot = ri / preinter;
    q = exp(log(R / h_dot / (preinter + x_eps)) / (Nx - preinter - x_eps));
    for (int i = 0; i < Nx; i++) {
        if (i < preinter + x_eps) {
            x_vect[i] = h_dot * i;
        }
    ...
    for (int i = 0; i < Nx; i++) {
        ...
        //else if (i <= preinter + 4) {
        //    Cell_Properties_vector[i].T = (Tright - T_left) / pow((x_vect[preinter + 4] - x_vect[preinter]), step_pol)
        //        * pow((x_vect[i] - x_vect[preinter]), step_pol) + T_left;
        //}
        else {
...
double F_right_rho_1(UserData data,
    double rho_l, double rho, double rho_r, double r_l, double r, double r_r,
    double u_l, double u, double u_r,
    int number_cell) {
    //return - 2 / (r_r - r_l) / pow(r, 2) * (pow(0.5 * (r_r + r), 2.) * (rho_r * u_r + rho * u) / 2. - pow(0.5 * (r + r_l), 2.) * (rho * u + rho_l * u_l) / 2.);
...
int KinSetIc(int NEQ)
{
    ...
    cout << "Mu = " << get_W(Cell_Properties_inter.Y.data()) << "\n";
    double mol_w = my_mol_weight(komponents[Fuel]);
    cout << "Pf/P * mol_w/W = " << Pf(Cell_Properties_inter.T, r_inter) / P * mol_w / get_W(Cell_Properties_inter.Y.data()) << "\n";
